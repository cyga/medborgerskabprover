...
4. В Telegram открой своего бота → `/start` → `/reminder` (напоминание вечером).
...
`/reminder` — ежедневное напоминание. Без аргумента показывает кнопки,
`/reminder 21:15` ставит своё время, `/reminder off` выключает.
Время — по Копенгагену, перевод часов учитывается.
